...
/**
 * @dev {SCPNSGpuTypes} token, including:
 *
...
contract SCPNSGpuTypes is 
    ...
    mapping (bytes32 => uint256) private _name2IDs;
    // Mapping from id to name
    mapping (uint256 => bytes32) private _id2Names;
    ...
    function initialize(string memory name_, string memory symbol_, string memory baseURI_) public virtual initializer {
        __SCPNSGpuTypes_init(name_, symbol_, baseURI_);
    }
    /**
     ...
     */
    function __SCPNSGpuTypes_init(string memory name_, string memory symbol_, string memory baseURI_) internal initializer {
        __Pausable_init_unchained();
        __AccessControlEnumerable_init_unchained();
        __SCPNSGpuTypes_init_unchained(name_, symbol_, baseURI_);
    }

    function __SCPNSGpuTypes_init_unchained(string memory name_, string memory symbol_, string memory baseURI_) internal initializer {
        baseURI= baseURI_;
        name = name_;
        symbol = symbol_;
    ...
    function mint(uint256 tokenId, bytes32 name_, string memory datas) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "SCPNSGpuTypes: must have minter role to mint");
        require(_name2IDs[name_] == uint256(0), "name is exists.");
        require(_id2Names[tokenId] == bytes32(0), "tokenId is exists.");
    ...
    */
    ...
    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "SCPNSGpuTypes: must have pauser role to pause");
        _pause();
    }
    ...
    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "SCPNSGpuTypes: must have pauser role to unpause");
        _unpause();
    }
    ...
    */
    ...
    function tokenOfByIndex(uint256 index) public view virtual returns(uint256) {
        require(index < _allTokens.length, "SCPNSGpuTypes: index out of bounds.");
