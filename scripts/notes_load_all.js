...
async function read(client, block_number, index, max_count, msgs)
{
    ...
    filter = await client.filters.Write(null, index, null);
    ...
    if (max_count > 0) {
        index--;
        await read(client, pre_block_number.toNumber(), index, max_count, msgs);
    }
}
...
async function run() {
    ...
    //write()
    block_number = await cobj.lastBlockNumber();
    ...
    msgs = []
    await read(cobj, block_number.toNumber(), count - 1, 10, msgs);
